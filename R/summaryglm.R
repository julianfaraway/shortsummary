...
#' @param show.nobs logical. If TRUE, print n (number of observations) at the bottom.
...
print.summary.glm <-
            ...
            concise = TRUE, show.nobs = FALSE, ...)
  {
    ...
    # Only print the dispersion line for families where it is not fixed at 1
    fixed_dispersion_families <- c("binomial", "poisson", "Negative Binomial")
    family_name <- x$family$family
    show_dispersion <- !concise ||
      !any(vapply(fixed_dispersion_families, startsWith, logical(1L), x = family_name))
    if (show_dispersion) {
      cat("\n(Dispersion parameter for", family_name,
...
#'
#' @param x an object of class "glm", usually, a result of a call to glm().
#' @param digits the number of significant digits to use when printing.
#' @param ... further arguments passed to or from other methods.
#' @return Nothing - prints output only.
#' @export print.glm
