...
#' @param show.nobs logical. If TRUE, print n and p at the bottom (similar to faraway::sumary).
            ...
            show.nobs = FALSE, ...)
  {
    ...
    if (length(x$aliased) == 0L) {
      ...
      printCoefmat(coefs, digits = digits, signif.stars = signif.stars,
                   ...
                   na.print = "NA",
                   eps.Pvalue = if (!concise) .Machine$double.eps else 1e-4, ...)
    }
    ...
    if (!is.null(x$fstatistic)) {
      ...
      cat("F-statistic:", formatC(x$fstatistic[1L], digits = digits),
          "on", x$fstatistic[2L], "and", x$fstatistic[3L], "DF,  p-value:",
          format.pval(pf(x$fstatistic[1L], x$fstatistic[2L], x$fstatistic[3L],
                         lower.tail = FALSE),
                      digits = digits,
                      eps = if (!concise) .Machine$double.eps else 1e-4))
      cat("\n")
    }
    if (show.nobs) {
      n <- df[1L] + df[2L]
